...
def load_dmatrix(somobj):
    """
    Load localadj matrix. It contains the Umatrix distances between
    input sequences
    """
...
def split_data(types,bmus,del_unclassified,titles=[]):
    ...
    """
    idxs_unclassified = np.squeeze(np.asarray(
        np.where(types == del_unclassified)))
    idxs_classified = np.squeeze(np.asarray(
        np.where(types != del_unclassified)))
    ...
    if len(titles) > 0:
        ...
        return idxs_unclassified, idxs_classified, \
            types_unclassified, types_classified, \
            bmus_unclassified, bmus_classified, \
            titles_unclassified, titles_classidied
    else:
        return idxs_unclassified, idxs_classified, types_unclassified, \
            types_classified, bmus_unclassified, bmus_classified
...
def get_bmu_type_dic(bmus, types, del_unclassified):
    ...
    for i, bmu in enumerate(bmus):
        if bmu not in bmu_type:
            ...
            if bmu_type[bmu] == types[i]:
                pass
            else:
                if bmu_type[bmu] == del_unclassified:
                    bmu_type[bmu] = types[i]
                elif types[i] == del_unclassified:
                    pass
                else:
                    print('In the %d BMU there are sequences from %s and %s' %
                          (bmu, bmu_type[bmu], types[i]))
                    if bmu_type[bmu] == 'A-delta':
                        bmu_type[bmu] == types[i]
                    else:
                        continue
    return bmu_type
...
class KNeighborsBMU(object):
    ...
    def score(self, test_idxs, test_types):
        ...
        score = sum(1 for x, y in zip(test_types, predict_types)
                    if x == y) / len(test_types)
        return score

    def predict(self, idx, idxs_toinf=[]):
        kclosest_types = []
        dist_row = self.dmatrix[idx, :]
        # Set to inf the distances of unclassified and the ones from
        # idxs_toinf (if not None)
        for _idx in self.unclassified_idxs:
            if _idx in self.dic_types_train.keys():
                pass
            else:
        ...
        # Get the types of the remaining k-th closest bmusidx in the
        # training set
        i = 0
