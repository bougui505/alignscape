...
def get_shortestPath(graph, start, end):
    sdist, pred = csgraph.shortest_path(graph, directed=False,
                                        indices=(start, end),
...
def get_uumat_ravel_cell(rcell, umatshape, uumatshape, mapping):
    """
    From a ravel cell of the umat obtain the correspinding ravel
    cell of the uumat umatshape and uumatshape are 2D list
    """
...
def get_cells_copies(shape, cell):
    i, j = cell
    n1, n2 = shape
    cells = [(i-n1, j-n2), (i-n1, j), (i-n1, j+n2), (i, j-n2),
             (i, j), (i, j+n2), (i+n1, j-n2), (i+n1, j), (i+n1, j+n2)]
    return cells
...
def remove_uumat_offset(uumat, mapping, reversed_mapping):
    ...
    for i in range(n1):
        ...
        if (sum(rowinf) is n1) and (row_break[0] is True):
            row_offset[0] += 1
        else:
            row_break[0] = False
        if (sum(colinf) == n2) and (col_break[0] is True):
            col_offset[0] += 1
        else:
            col_break[0] = False
    for i in range(n1-1, -1, -1):
        ...
        rowinf = np.isinf(row).astype(int)
        colinf = np.isinf(col).astype(int)
        if (sum(rowinf) == n1) and (row_break[1] is True):
            row_offset[1] += 1
        else:
            row_break[1] = False
        if (sum(colinf) == n2) and (col_break[1] is True):
            col_offset[1] += 1
        else:
            col_break[1] = False
    ...
    for k in mapping:
        mapping[k] = tuple(np.asarray(mapping[k]) -
...
def get_unfold_umat(umat, adj, qbmus, mstree, offset=True):
    """
    Unfold a U-Matrix (umat) using a minimal spanning tree (mstree)
    between BMUS (bmus)
    """
    ...
    # Reorder mstree_pairs and mstree_data dinamicaly to start from the
    # closest pair of qbmus
    dmstree_pairs = [[], []]
    dmstree_data = []
    ...
    qbmus_ravel = np.ravel_multi_index(np.asarray(list(mapping.keys())).T,
                                       (n1, n2))
    sdist, pred = csgraph.shortest_path(adj, indices=qbmus_ravel,
                                        directed=False,
                                        return_predecessors=True)
    ...
    if offset:
        uumat, mapping, reversed_mapping = \
            remove_uumat_offset(uumat, mapping, reversed_mapping)
...
def get_localadjmat(umat, adjmat, bmus, verbose=True):
    """
    Get all paths and path distances for all combinations of queries and
    generate a new graph of shortest distances between queries
    """
    n1, n2 = umat.shape
    ...
    for pair in itertools.permutations(indxbmus, 2):
        ...
        pbar.update(1)
    pbar.close()
    localadj = scipy.sparse.coo_matrix((localadj['data'], (localadj['row'],
                                                           localadj['col'])))
...
def get_unfold_mstree(mstree_pairs, mstree_paths, somsize,
                      unfsomsize, mapping):
    #dok = uadj.todok()
    unf_mstree_pairs = []
    unf_rpaths = {}
    ...
    for pair in mstree_pairs:
        unf_rpair = [get_uumat_ravel_cell(pair[0], somsize,
                                          unfsomsize, mapping),
                     get_uumat_ravel_cell(pair[1], somsize, unfsomsize,
                                          mapping)]
    ...
    for k in mstree_paths:
        unf_rk = (get_uumat_ravel_cell(k[0], somsize, unfsomsize, mapping),
                  get_uumat_ravel_cell(k[1], somsize, unfsomsize, mapping))
        unf_rpath = [get_uumat_ravel_cell(step, somsize, unfsomsize, mapping)
                     for step in mstree_paths[k]]
...
def from_mstree_to_graph(mstree, bmus, labels, somsize):
    ...
    mstree_nodes_labels = [';'.join(node_label).replace(">", "") for
                           node_label in mstree_nodes_labels]
