...
def jax_score_matrix_vec(vec1, vec2, b62, gap_s=-5, gap_e=-1):
    ...
    final_scores = scores + max_gaps_aggregated * gap_s +\
...
def jax_rscore_matrix_vec(vec1, vec2, b62, gap_s=-5, gap_e=-1):
    ...
    if vec2.ndim == 2:
        vec2 = vec2[None, ...]
    nchars = len(aalist)
    rscore = jnp.shape(vec1)[1] * ((b62.sum()) + nchars * (gap_s + gap_e))\
        / (jnp.shape(b62)[0] * jnp.shape(b62)[1] + nchars * 2)
    rscores = jnp.tile(rscore, (jnp.shape(vec1)[0], jnp.shape(vec2)[0]))
    return rscores


def jax_iscore_matrix_vec(vec1, b62, gap_s=-5, gap_e=-1):
    """
    Jax Implementation
    """
    if vec1.ndim == 2:
