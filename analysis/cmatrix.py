...
class Cmatrix(object):
    ...
    Correlation matrix
    """
    def __init__(self, dmatrices, labels, outname=None, rand=True,
                 replacement=True, sampling=1, tolerance=math.inf):
        """
        """
    ...
    def get_correlation(self, df1, df2, n1, n2):
        ...
        if len(newcolumns) < 3:
            warnings.warn('Not enough common elements between %s and %s \
                          (at least 3 needed)' % (n1, n2), UserWarning)
            return 0
        ...
        # Sampling
        if self.sampling != 1:
            indexsampl = random.sample(range(len(vec1)),
                                       int(len(vec1)*self.sampling))
        ...
        if self.tolerance != math.inf:
            ...
            if len(np.argwhere(difvec > self.tolerance)) != 0:
                indextolerance = np.concatenate(np.argwhere(difvec >
                                                            self.tolerance))
    ...
    def plot(self, vmin=None, vmax=None):
        cg = sns.clustermap(self.cdf, cmap="BrBG", linewidths=0.30,
                            metric='euclidean', vmin=vmin, vmax=vmax)
